...
<div class="rounded-3xl shadow ring-1 ring-slate-200 bg-white overflow-hidden">
    ...
    <div class="px-6 py-6 bg-gradient-to-r from-maroon-700 via-maroon-600 to-yellow-600">
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
            <div class="text-white">
                ...
                <p class="text-sm text-white/85 mt-1">Kelola akun, role, dan divisi untuk akses ERP.</p>
            </div>
    ...
    {{-- ALERTS (single, pretty, no-duplicate) --}}
    @php
    ...
    <div class="px-6 pt-5">
        ...
        {{-- 1) PASSWORD BANNER (highest priority) --}}
        @if ($hasPwd)
        ...
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-200">
                ...
                <tr class="hover:bg-slate-50" x-data="{open:false, confirmReset:false, confirmDelete:false}">
                    ...
                    <td class="px-4 py-3">
                        @if($u->division)
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-indigo-100 text-indigo-700 ring-1 ring-indigo-200">
                            {{ $u->division->name }}
                        </span>
                        @else
                    ...
                    <td class="px-4 py-3">
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="relative inline-block text-left">
                            <button @click="open=!open"
                                class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold">
                                Actions
                        ...
                        {{-- Forms tersembunyi --}}
                        <form x-ref="resetForm" method="POST" action="{{ route('admin.users.resetPassword',$u) }}" class="hidden">
                            @csrf
                        </form>
                        ...
                        {{-- MODAL: Reset Password (soft, animated) --}}
                        <div x-cloak x-show="confirmReset" x-transition.opacity.duration.200ms class="fixed inset-0 z-40" role="dialog" aria-modal="true" aria-labelledby="resetTitle"
                            ...
                            <div class="absolute inset-0 flex items-center justify-center p-4">
                                <div
                                    ...
                                    <div class="px-5 pb-4">
                                        ...
                                        <p class="mt-2.5 text-[13px] text-slate-500">User akan menerima password baru yang tampil sekali di halaman ini.</p>
                                    </div>
                        ...
                        {{-- MODAL: Delete (soft, animated, matches reset style + safeguards) --}}
<div x-cloak x-show="confirmDelete" x-transition.opacity.duration.200ms class="fixed inset-0 z-40" role="dialog" aria-modal="true" aria-labelledby="delTitle"
  ...
  <div class="absolute inset-0 flex items-center justify-center p-4">
    <div
      ...
      x-init="$nextTick(()=> $el.querySelector('[data-primary]')?.focus())"
    >
      {{-- HEADER (icon + title + close) --}}
      <div class="px-5 pt-5 pb-3 flex items-start gap-3">
        <div class="h-10 w-10 rounded-xl bg-gradient-to-br from-red-500 to-rose-500 text-white flex items-center justify-center shadow-inner">
          <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      <div class="px-5 pb-4 space-y-3">
        ...
        <label class="flex items-start gap-2 text-sm text-slate-700">
          <input type="checkbox" class="mt-0.5 rounded border-slate-300 text-rose-600 focus:ring-rose-500"
                 x-model="ack">
                ...
                <tr>
                    <td colspan="6" class="px-4 py-16"> {{-- was 5; now 6 because Site col added --}}
                        <div class="mx-auto max-w-md text-center">
                            ...
                            <a href="{{ route('admin.users.create') }}"
                                class="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl font-semibold shadow-sm hover:shadow bg-gold-500 text-maroon-900">
                                + Tambah User
                            </a>
                        </div>
                    </td>
            ...
            <div class="flex items-start justify-between gap-2">
                ...
                <div class="text-right space-y-2">
                    ...
                    <button type="button" @click="confirmDelete=true"
                        class="w-full px-3 py-1.5 rounded-lg bg-red-600 text-white hover:bg-red-500 text-xs font-medium">
                        🗑️ Delete
                    </button>
                </div>
            </div>

            {{-- hidden forms per card --}}
            <form x-ref="resetForm" method="POST" action="{{ route('admin.users.resetPassword',$u) }}" class="hidden">
                @csrf
            </form>
            <form x-ref="deleteForm" method="POST" action="{{ route('admin.users.destroy',$u) }}" class="hidden">
            ...
            <div x-cloak x-show="confirmReset" class="fixed inset-0 z-30" aria-modal="true" role="dialog">
                ...
                <div class="absolute inset-0 flex items-center justify-center p-4">
                    <div class="w-full max-w-sm rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 overflow-hidden">
                        <div class="px-4 py-3 bg-gradient-to-r from-maroon-700 via-maroon-600 to-yellow-600 text-white text-sm font-semibold">
