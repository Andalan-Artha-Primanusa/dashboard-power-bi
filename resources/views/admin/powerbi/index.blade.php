    ...
    <form method="GET" class="grid gap-3 sm:grid-cols-12 items-center">
      ...
      <label class="sm:col-span-6 relative">
        <input name="q" value="{{ $q }}" placeholder="Cari nama atau URL…"
               class="w-full rounded-xl border-slate-300 pl-10 pr-3 py-2.5 focus:ring-gold-500 focus:border-gold-500" />
      ...
      <div class="sm:col-span-4">
        <select name="status"
                class="w-full rounded-xl border-slate-300 px-3 py-2.5 focus:ring-gold-500 focus:border-gold-500">
      ...
      <div class="sm:col-span-2 sm:justify-self-end">
        <button class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-maroon-700 text-white font-medium hover:bg-maroon-600">
          Terapkan
        </button>
      </div>
    </form>
            ...
            <td class="px-4 py-3 align-top text-right">
              ...
              <div x-data="{open:false}" class="relative inline-block text-left">
                ...
                <div x-cloak x-show="open" @click.outside="open=false"
                  ...
                  <a href="{{ $report->embed_url }}" target="_blank" class="block px-3 py-2 rounded-lg hover:bg-slate-50 text-sm">🔗 Open Embed</a>
                  @if($isDeleted)
                  ...
                  @else
                    <form action="{{ route('admin.powerbi.destroy',$report) }}" method="POST" onsubmit="return confirm('Yakin hapus report ini?')">
                      @csrf @method('DELETE')
                      <button class="w-full text-left px-3 py-2 rounded-lg text-red-700 hover:bg-red-50 text-sm">🗑️ Delete</button>
                    </form>
                  @endif
                </div>
              </div>
      ...
      <div class="p-4">
        <div class="flex items-start justify-between gap-2">
          ...
          <div x-data="{open:false}" class="relative">
            ...
            <div x-cloak x-show="open" @click.outside="open=false"
                 class="absolute right-0 mt-2 w-44 rounded-xl border bg-white shadow-lg p-1 text-sm">
              <a href="{{ route('admin.powerbi.edit',$report) }}" class="block px-3 py-2 rounded-lg hover:bg-slate-50">✏️ Edit</a>
              <a href="{{ $report->embed_url }}" target="_blank" class="block px-3 py-2 rounded-lg hover:bg-slate-50">🔗 Open Embed</a>
              @if($isDeleted)
                <form action="{{ route('admin.powerbi.restore',$report->id) }}" method="POST">@csrf
              ...
              @else
                <form action="{{ route('admin.powerbi.destroy',$report) }}" method="POST" onsubmit="return confirm('Yakin hapus report ini?')">
                  @csrf @method('DELETE')
                  <button class="w-full text-left px-3 py-2 rounded-lg text-red-700 hover:bg-red-50">🗑️ Delete</button>
                </form>
              @endif
            </div>
          </div>
