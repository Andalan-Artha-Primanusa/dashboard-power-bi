...
@php
  ...
  $showDel = $showDel ?? (bool) request('with_trashed');
@endphp
    ...
    <div class="px-4 py-4 sm:px-5 sm:py-4">
      ...
      <div class="grid gap-3 md:grid-cols-12 items-end">
        ...
        <div class="md:col-span-6">
          ...
          <div class="mt-1 relative">
            <input type="text" name="q" value="{{ $q ?? '' }}"
                   ...
                   class="w-full rounded-xl border border-slate-300 bg-white pl-10 pr-9 py-2.5 text-sm
            ...
            @if(($q ?? '') !== '')
              <button type="button"
                      ...
                      class="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg hover:bg-slate-100 text-slate-500">
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path d="M18 6 6 18M6 6l12 12"/>
                </svg>
              </button>
        ...
        <div class="md:col-span-3">
          ...
          <select name="status"
                  class="mt-1 w-full rounded-xl border border-slate-300 bg-white py-2.5 text-sm
                         focus:border-maroon-700 focus:ring-2 focus:ring-maroon-700/80">
        ...
        {{-- Button --}}
        <div class="md:col-span-1 flex items-center gap-2 mt-3 md:mt-0">
          <button type="submit"
                  class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-maroon-700 text-white text-sm font-semibold hover:bg-maroon-800 ring-1 ring-maroon-900/20">
            Terapkan
          </button>
        </div>
      </div>
          ...
          @forelse($divisions as $d)
            ...
            <tr class="{{ $isDeleted ? 'bg-rose-50/40' : 'hover:bg-slate-50' }}"
              ...
              <td class="px-4 py-3 text-right">
                ...
                <form x-ref="deleteFormSoft" method="POST" action="{{ route('admin.divisions.destroy',$d) }}" class="hidden">
                  @csrf @method('DELETE')
                </form>
            

                {{-- DELETE MODAL --}}
                <div x-cloak x-show="confirmDelete" x-transition.opacity.duration.200ms
                     class="fixed inset-0 z-40" role="dialog" aria-modal="true" aria-labelledby="delTitle-{{ $d->id }}"
                     @keydown.escape.window="confirmDelete=false">
                  <div class="absolute inset-0 bg-black/40 backdrop-blur-[2px]"
                       @click="confirmDelete=false" x-transition.opacity.duration.200ms></div>
                    ...
                    <div x-show="confirmDelete"
                      ...
                      <div class="px-5 pt-5 pb-3 flex items-start gap-3">
                        ...
                        <button data-cancel @click="confirmDelete=false"
                                class="ml-auto rounded-lg p-1.5 text-slate-500 hover:bg-slate-100/70">
    ...
    <div class="md:hidden divide-y bg-white">
      @forelse($divisions as $d)
        ...
        <div class="p-4" x-data="{ open:false, confirmDelete:false }">
          <div class="flex items-start justify-between gap-3">
            <div>
              ...
              <div class="text-xs text-slate-500">
                Kode: <span class="font-mono">{{ $d->code }}</span>
              </div>
              <div class="mt-1 space-x-1">
                ...
                @endif
                @if($isDeleted)
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] bg-rose-100 text-rose-700 ring-1 ring-rose-200">Terhapus</span>
                @endif
              </div>
            </div>
            ...
            <div class="relative">
              <button @click="open=!open"
                ...
                Actions
                <svg class="inline h-3.5 w-3.5 ml-1 align-[-2px]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
              </button>
              <div x-cloak x-show="open" @click.outside="open=false"
          ...
          {{-- Modal --}}
          <div x-cloak x-show="confirmDelete" x-transition.opacity.duration.200ms
               class="fixed inset-0 z-40" role="dialog" aria-modal="true">
            <div class="absolute inset-0 bg-black/40 backdrop-blur-[2px]"
                 @click="confirmDelete=false" x-transition.opacity.duration.200ms></div>
            <div class="absolute inset-0 flex items-center justify-center p-4">
              <div x-show="confirmDelete"
