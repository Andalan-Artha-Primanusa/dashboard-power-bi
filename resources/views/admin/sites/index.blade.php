...
<div class="rounded-3xl overflow-hidden shadow ring-1 ring-slate-200 bg-white">
  ...
  {{-- ALERTS (seragam dengan Divisions) --}}
  @php
  ...
  <div class="px-6 py-4 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
    ...
    <form method="GET" class="w-full lg:w-auto">
      ...
      <div class="relative">
        ...
        @endif
        <button class="absolute right-1.5 top-1/2 -translate-y-1/2 inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-maroon-700 text-white text-sm font-semibold hover:bg-maroon-600">
          Cari
        </button>
      </div>
    </form>
        ...
        @forelse($sites as $s)
          <tr class="{{ (method_exists($s,'trashed') && $s->trashed()) ? 'bg-rose-50/40' : 'hover:bg-slate-50' }}"
            ...
            <td class="px-4 py-3 text-right">
              ...
              {{-- DELETE MODAL (SERAGAM dgn Divisions) --}}
              <div x-cloak x-show="confirmDelete" x-transition.opacity.duration.200ms class="fixed inset-0 z-40" role="dialog" aria-modal="true" aria-labelledby="delTitle-{{ $s->id }}"
                ...
                <div class="absolute inset-0 flex items-center justify-center p-4">
                  <div x-show="confirmDelete"
                    ...
                    <div class="px-5 pt-5 pb-3 flex items-start gap-3">
                      ...
                      <div class="min-w-0">
                        <h3 id="delTitle-{{ $s->id }}" class="text-base font-semibold text-slate-900">
