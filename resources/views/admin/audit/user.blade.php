    ...
    <div class="relative px-6 py-6 sm:px-8 sm:py-7 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-white">
      ...
      <div class="flex flex-wrap items-center gap-2 justify-start sm:justify-end">
        <a href="{{ route('admin.audit.index') }}"
           class="px-3 py-2 rounded-xl text-sm font-semibold shadow-sm hover:shadow bg-white text-maroon-900 ring-1 ring-white/20">
          ← Semua Log
        </a>
      ...
      <form method="GET" class="grid gap-3 sm:grid-cols-12 items-center">
        <label class="sm:col-span-6 relative">
          <input
            ...
            name="q"
            value="{{ $q ?? '' }}"
            placeholder="Cari payload / action…"
            ...
            <tr class="hover:bg-slate-50 align-top">
              <td class="px-4 py-3 whitespace-nowrap">
                ...
                <div class="text-xs text-slate-500">
                  {{ optional($l->created_at)->diffForHumans() }}
                </div>
              </td>
