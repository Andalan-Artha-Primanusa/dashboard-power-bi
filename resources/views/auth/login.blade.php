    ...
    <section class="relative flex w-full lg:w-[50%] items-center justify-center border-b lg:border-b-0 lg:border-r border-slate-200/60 bg-white/80 backdrop-blur">
      <div class="w-full max-w-md px-6 sm:px-8 py-12 sm:py-16">
        ...
        <div class="mb-8 flex items-center gap-4">
          {{-- LOGO ARCA (LEBIH BESAR) --}}
          <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-white ring-1 ring-slate-200 shadow-md overflow-hidden">
            <img
          ...
          <div>
            <h1 class="text-[18px] font-bold text-[var(--maroon-800)] leading-tight">
              ARCA — Andalan Reporting &amp; Control Analytics
            </h1>
            <p class="text-[12px] text-[var(--maroon-700)]/75">
              Portal pelaporan &amp; kendali internal Andalan Group
            </p>
          </div>
        </div>

        {{-- Headline & subheadline (bahasa resmi) --}}
        <h2 class="text-3xl sm:text-4xl font-black tracking-tight text-[var(--maroon-900)]">
          Masuk ke Portal ARCA
        </h2>
        <p class="mt-1 text-[13px] sm:text-sm text-slate-600">
          Silakan masuk menggunakan akun surel resmi Andalan Group untuk mengakses layanan pelaporan dan kendali perusahaan.
        </p>
        ...
        <div x-data="{ show:false }" class="mt-6 relative rounded-[1.6rem] bg-white ring-1 ring-slate-200 shadow-xl overflow-hidden">
          ...
          <form method="POST" action="{{ route('login') }}" class="p-6 sm:p-8 space-y-5 relative z-10">
            ...
            {{-- Alamat Surel --}}
            <div>
              <label for="email" class="block text-[13px] font-semibold text-[var(--maroon-800)]">
              ...
              @error('email')
                <p class="mt-2 text-sm text-[var(--maroon-700)]">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div>
              <div class="flex items-center justify-between">
                <label for="password" class="block text-[13px] font-semibold text-[var(--maroon-800)]">
                  Kata Sandi
                </label>
                @if (Route::has('password.request'))
                  <a href="{{ route('password.request') }}"
                     class="text-[12px] font-medium text-[var(--maroon-700)] hover:text-[var(--maroon-900)]">
                    Lupa kata sandi?
                  </a>
                @endif
              </div>
              <div class="mt-2 relative">
                <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center">
                  <svg class="h-[18px] w-[18px] text-[var(--maroon-600)]/85" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="hidden sm:flex items-center gap-1 text-[11px] text-slate-500">
                <svg class="h-[14px] w-[14px]" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2a10 10 0 1 1 0 20 10 10 0 0 1 0-20Zm1 5h-2v6l5 3 .9-1.45L13 12.2V7Z"/>
                </svg>
                <span>Sesi login terenkripsi &amp; diaudit</span>
              </div>
            </div>
            ...
            <p class="mt-3 text-[11px] text-slate-500 leading-relaxed">
              Dengan mengakses portal ini, Anda menyatakan memahami dan mematuhi kebijakan keamanan informasi serta ketentuan penggunaan sistem di lingkungan Andalan Group.
            </p>
          </form>
        </div>

        <p class="mt-6 text-[11px] text-slate-500">
          © {{ date('Y') }} Andalan Group. Seluruh hak cipta dilindungi. Hanya untuk penggunaan internal.
        </p>
      </div>
    </section>
    ...
    <aside class="relative w-full lg:w-[50%]">
      ...
      <div class="lg:hidden">
        <div class="relative h-48 w-full overflow-hidden rounded-b-[2rem]">
          ...
          {{-- logo kecil di mobile (dibesarkan) --}}
          <div class="absolute bottom-3 left-4 inline-flex items-center gap-2 rounded-full bg-black/40 px-3 py-1.5 backdrop-blur">
            <img src="{{ asset('assets/images/logoarca.png') }}" alt="Logo ARCA" class="h-6 w-6 object-contain">
            <span class="text-[11px] text-white/90">ARCA Portal</span>
          </div>
        </div>
      </div>
      ...
      <div class="relative hidden h-full w-full lg:block">
        ...
        {{-- Floating logo chip (dibesarkan) --}}
        <div class="absolute top-8 left-10 flex items-center gap-3">
          <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/95 shadow-lg overflow-hidden">
            <img src="{{ asset('assets/images/logoarca.png') }}" alt="Logo ARCA" class="h-12 w-12 object-contain">
          </div>
          <div class="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 ring-1 ring-white/20 backdrop-blur">
            <span class="h-2 w-2 rounded-full bg-emerald-300"></span>
            <span class="text-xs text-white/90">ARCA — Akses Aman &amp; Terkendali</span>
          </div>
        </div>

        {{-- Headline & tags (netral lintas divisi) --}}
        <div class="absolute bottom-12 left-12 right-12 text-white">
          <h3 class="text-3xl xl:text-4xl font-black drop-shadow-sm leading-tight">
            Satu Gerbang Pelaporan &amp; Kendali Perusahaan
          </h3>
          <p class="mt-2 text-sm text-white/85 max-w-xl">
            Layanan terpusat untuk pelaporan, pemantauan, dan pengendalian kinerja lintas entitas dalam ekosistem Andalan Group.
          </p>
          <div class="mt-4 flex flex-wrap gap-2">
            <div class="text-[11px] px-3 py-1 rounded-full bg-white/10 ring-1 ring-white/20 backdrop-blur">Multi-Situs &amp; Multi-Entitas</div>
            <div class="text-[11px] px-3 py-1 rounded-full bg-white/10 ring-1 ring-white/20 backdrop-blur">Akses Berbasis Peran</div>
            <div class="text-[11px] px-3 py-1 rounded-full bg-white/10 ring-1 ring-white/20 backdrop-blur">Jejak Audit Terintegrasi</div>
          </div>
        </div>
      </div>
    </aside>
